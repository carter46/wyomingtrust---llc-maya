...
<?php
$sendBackHref = $coinKeyParam !== ''
    ? 'asset-detail.php?coin_key=' . rawurlencode($coinKeyParam) . ($trustIdParam > 0 ? '&trust_id=' . $trustIdParam : '')
    : 'assets.php';
$sendBackLabel = $coinKeyParam !== '' ? 'Back to Asset' : 'Back to Assets';
?>
<a href="<?php echo escape_html($sendBackHref); ?>" class="inline-flex items-center gap-1 text-secondary font-label-md text-label-md hover:underline mb-4">
<?php echo wt_icon('arrow-back', 'w-4 h-4'); ?> <?php echo escape_html($sendBackLabel); ?>
</a>
